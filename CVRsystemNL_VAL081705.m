...
LVpar = [400. 0.68  40.  0.  0.10 60.  10. -5.]; 
RVpar = [125. 0.68  40.  0.  0.07 60.  10. -5.];  
LApar = [20.  0.19  0.15 30. 0.   0.07 65. 10. -5.];
RApar = [20.  0.18  0.15 30. 0.   0.06 65. 10. -5.];
...
[t,y] = ode23tb(@cardiolumpedCVRNL,time,y0',[],PV_LV,PV_RV,PV_LA,PV_RA,LVpar,RVpar,LApar,RApar,...
                   ...
                   SVpar,PApar,hr,vd,rho,CaLV,CaRV,rvlvd,nc,pRice,Fmax,time(1));
...
t1 = t2 - 1.0/dt; 
time = t(t1:t2);
figure, plot(time,y(t1:t2,2)/MVpar(2),time,y(t1:t2,3)/AVpar(2))
legend('MV','AV',2);
xlabel('time (s)');
ylabel('velocity (cm/s)');
figure, plot(time,y(t1:t2,6)/TVpar(2),time,y(t1:t2,7)/PVvpar(2))
legend('TV','PV',2);
xlabel('time (s)');
ylabel('velocity (cm/s)');

figure, plot(time,p(t1:t2,1),time,p(t1:t2,2),time,p(t1:t2,3))
legend('LA','LV','SA',2);
xlabel('time (s)');
ylabel('pressure (mmHg)');
figure, plot(time,p(t1:t2,5),time,p(t1:t2,6),time,p(t1:t2,7))
legend('RA','RV','PA',2);
xlabel('time (s)');
ylabel('pressure (mmHg)');

figure, plot(time,y(t1:t2,9),time,y(t1:t2,10))
legend('LA','LV',2);
xlabel('time (s)');
ylabel('volume (ml)');
figure, plot(time,y(t1:t2,13),time,y(t1:t2,14))
legend('RA','RV',2);
xlabel('time (s)');
ylabel('volume (ml)');
...
legend('TCA','N0','N1','P0','P1','P2','P3',2);
xlabel('time (s)');
